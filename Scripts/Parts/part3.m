...
[tt,r_t,r_dot_t] = solve2bodyK_part3(1,[rE_target./r2;rE_dot_target./r2./n2],tspan);
r_t=r_t*r2; % dimensional position target
tt=tt/n2; % dimensional time

% initial condition chaser in Equatorial
r_c0=rH_err+[r2;0;0]; 
r_dot_c0=rH_dot_err+[0;n2*r2;0]+deltaV1*r2*n2+cross([0;0;n2],rH_err);
% adimensional propagation
[t,r_c,r_dot_c] = solve2bodyK_part3(1,[r_c0/r2;r_dot_c0/r2/n2],tspan);
r_c=r_c*r2; % dimensional position chaser
r_dot_c=r_dot_c*r2*n2; % dimensional velocity chaser
t=t/n2; % dimensional time = tt

% rendezvous in Equatorial restircted 2bodyK (no perturbation)
figure;
hold on
plot(r_t(1,:),r_t(2,:),"r")
...
title("Chaser difference in position due to drag")
xlabel("t [s]")
ylabel("[km]")
...
axis equal
title("Rendezvous with chaser drag")
xlabel("x [km]")
ylabel("y [km]")
legend("Target","Starting points","Ending points","Chaser")
hold off
...
fprintf('Final position offset target - chaser with drag: %.4f [m]\n', norm([csi(end);eta(end)],2)*r2*1000);
fprintf('Final velocity offset target - chaser with drag: %.4f [m/s]\n', norm([v_csi(end)+deltaV2(1);v_eta(end)+deltaV2(2)],2)*r2*1000);
end